...
function authErrorMessage(error: unknown, fallback: string) {
    if (error instanceof ApiError) {
        if (error.status === 401) return "Fel email eller lösenord.";
...
export default function LoginScreen() {
    ...
    async function submit() {
        if (!email.trim() || !password) {
            setError("Fyll i email och lösenord.");
            return;
        }
        ...
        try {
            ...
            setError(authErrorMessage(submitError, "Kunde inte logga in. Försök igen."));
        } finally {
                ...
                <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.keyboard}>
                    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scrollContent}>
                        <View style={styles.header}>
                            <AppText size={42}>WanderTale</AppText>
                            <AppText style={styles.subtitle}>Logga in och fortsätt där resan pausade.</AppText>
                        </View>

                        <View style={styles.paper}>
                            <View style={styles.tape}/>
                            <AppText style={styles.formTitle}>Logga in</AppText>
                            ...
                            <AuthInput icon="lock-closed-outline" placeholder="Lösenord" value={password} onChangeText={setPassword} secureTextEntry autoComplete="password"/>
                            ...
                            <Pressable onPress={() => void submit()} disabled={isSubmitting} style={({pressed}) => [styles.primaryButton, (pressed || isSubmitting) && styles.pressed]}>
                                {isSubmitting ? <ActivityIndicator color={tokens.background}/> : <AppText style={styles.primaryButtonText}>Logga in</AppText>}
                            </Pressable>

                            <Pressable onPress={() => router.push("/auth/register")} style={({pressed}) => [styles.linkButton, pressed && styles.pressed]}>
                                <AppText style={styles.linkText}>Inget konto än? Skapa ett</AppText>
                            </Pressable>
                        </View>
                    </ScrollView>
                </KeyboardAvoidingView>
...
const createStyles = (theme: ReturnType<typeof useTheme>["theme"]["tokens"]) => StyleSheet.create({
    ...
    subtitle: {marginTop: 6, textAlign: "center", fontSize: 19, color: theme.textSecondary},
    paper: {
        width: "100%",
        maxWidth: 420,
        alignSelf: "center",
    ...
    formTitle: {marginBottom: 14, textAlign: "center", fontSize: 30},
    inputRow: {
        minHeight: 48,
        marginBottom: 12,
